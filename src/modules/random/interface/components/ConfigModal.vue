<template>
  <v-slide-y-reverse-transition>
    <div v-if="visible" class="d-flex align-center justify-center bg-transparent" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; z-index: 100; background: rgba(0,0,0,0.6) !important; backdrop-filter: blur(2px);">
      <v-card class="sorteio-config-modal rounded-xl" width="100%" max-width="520" style="background: var(--card-bg, #ffffff); box-shadow: 0 10px 40px rgba(0,0,0,0.5); overflow: hidden;">
        ...
        <div style="background: var(--main-bg, #f5f5f5); padding: 24px; max-height: 60vh; overflow-y: auto;">
          ...
          <v-card class="settings-card rounded-xl pa-2 mb-6" flat style="background: var(--card-bg, #ffffff); box-shadow: var(--shadow);">
            <v-card-text class="pa-4">
              <div class="d-flex align-center justify-space-between mb-6">
                <div class="d-flex align-center">
                  ...
                  <div>
                    ...
                    <div class="text-caption" style="color: var(--sidebar-text-secondary);">Formatação dos nomes sorteados</div>
                  </div>
                </div>
              </div>
              ...
              <div class="d-flex align-center justify-space-between mb-3">
                <div class="d-flex align-center">
                  <v-icon size="18" color="primary" class="mr-2">mdi-format-letter-case</v-icon>
                  <span class="text-body-2 font-weight-bold" style="color: var(--sidebar-text);">Transformação de Texto</span>
                </div>
              </div>
              <v-btn-toggle v-model="localConfig.textTransform" mandatory divided variant="outlined" color="primary" class="w-100 mb-2 d-flex">
                <v-btn value="none" class="flex-grow-1 text-none text-caption font-weight-bold px-1">Aa (Normal)</v-btn>
                <v-btn value="uppercase" class="flex-grow-1 text-none text-caption font-weight-bold px-1">AA (Maiúsculo)</v-btn>
                <v-btn value="lowercase" class="flex-grow-1 text-none text-caption font-weight-bold px-1">aa (Minúsculo)</v-btn>
              </v-btn-toggle>
          ...
          <v-card class="settings-card rounded-xl pa-2" flat style="background: var(--card-bg, #ffffff); box-shadow: var(--shadow);">
            <v-card-text class="pa-4">
              <div class="d-flex align-center mb-6">
                <v-icon color="primary" class="mr-3" size="24">mdi-animation-outline</v-icon>
                <div>
                  <h3 class="font-weight-bold" style="color: var(--sidebar-text); font-size: 1.1rem; line-height: 1.2;">Dinâmica do Sorteio</h3>
                  <div class="text-caption" style="color: var(--sidebar-text-secondary);">Velocidade de rolagem dos nomes</div>
                </div>
              </div>

              <v-btn-toggle v-model="localConfig.animationSpeed" mandatory divided variant="outlined" color="primary" class="w-100">
                <v-btn value="fast" class="flex-grow-1 text-none font-weight-bold">
                  <v-icon start size="18">mdi-run-fast</v-icon>
                  Rápido
                </v-btn>
                <v-btn value="normal" class="flex-grow-1 text-none font-weight-bold">
                  <v-icon start size="18">mdi-run</v-icon>
                  Normal
                </v-btn>
